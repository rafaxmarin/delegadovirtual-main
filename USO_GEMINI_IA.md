...
La variable de entorno `GEMINI_API_KEY` le otorga capacidades de Inteligencia Artificial al bot mediante el modelo **Google Gemini (gemini-flash-latest)** (configurable mediante `GEMINI_MODEL`). A continuación se detallan las 6 funciones principales donde se utiliza la IA dentro del sistema:
